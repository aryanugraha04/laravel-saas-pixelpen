            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="nk-editor">
                        @php
                            // Decode JSON hanya sekali untuk efisiensi.
                            // Jika JSON tidak valid, $inputData akan menjadi array kosong untuk mencegah error.
                            ...
                            <div class="nk-editor-header">
                                <div class="nk-editor-title">
                                    <h4 class="me-3 mb-0 line-clamp-1">
                                        @if (isset($inputData['Article_Title']))
                                            {{ $inputData['Article_Title'] }}

                                        @elseif (isset($inputData['Blog_Ideas']))
                                            {{ $inputData['Blog_Ideas'] }}

                                        @elseif (isset($inputData['What_would_you_like_to_summarize']))
                                            {{ $inputData['What_would_you_like_to_summarize'] }}
                                        
                                        @elseif (isset($inputData['Blog_Titles']))
                                            {{ $inputData['Blog_Titles'] }}
                                        
                                        @elseif (isset($inputData['Paragraph_Description']))
                                            {{ $inputData['Paragraph_Description'] }}

                                        @elseif (isset($inputData['Include_your_text_here_to_check']))
                                            {{ $inputData['Include_your_text_here_to_check'] }}
                                            
                                        @else
                                            Untitled Document {{-- Teks cadangan jika tidak ada key yang cocok --}}
                                        @endif
                                    </h4>
                                </div>
