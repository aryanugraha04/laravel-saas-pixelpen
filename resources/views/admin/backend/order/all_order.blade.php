                    ...
                    <tr>
                        ...
                        <td class="tb-col tb-col-sm">
                            @if(strtolower($item->status) === 'pending')
                                ...
                                <div class="badge text-bg-warning-soft rounded-pill px-2 py-1 fs-6 lh-sm">{{ $item->status }}</div>

                            @elseif(strtolower($item->status) === 'paid')
                                {{-- Badge Hijau untuk status Paid --}}
                                <div class="badge text-bg-success-soft rounded-pill px-2 py-1 fs-6 lh-sm">{{ $item->status }}</div>
                            
                            @else
                                {{-- Badge Netral untuk status lainnya --}}
                                <div class="badge text-bg-secondary-soft rounded-pill px-2 py-1 fs-6 lh-sm">{{ $item->status }}</div>
                            @endif
                        </td>
                        <td class="tb-col tb-col-sm">
